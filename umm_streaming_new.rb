...
MASTODON_HOST = 'imastodon.net' #接続先インスタンス
TOKEN = ENV['MASTODON_ACCESS_TOKEN'] #.env内にMASTODON_ACCESS_TOKENを記述
...
def umm(id, reply_id, log_count)
    ...
    time = Time.now
    File.open("./log/streaming/" + log_count + ".txt", mode = "a+:utf-8:utf-8") do |log|
        log.puts(time)
        log.puts(id)
        log.puts("うみみ")
        log.puts "\n"
        if log.size >= 300
            ...
            File.open("./log/streaming/" + file_no.to_s + ".txt", mode = "a+:utf-8:utf-8")
        end
    end
    puts "うみみ" if @debug
end
...
def help(id, reply_id, log_count)
    help = File.open("./text/help.txt", mode = "a+:utf-8:utf-8").read
    toot = "@" + id + " " + help
    @rest.create_status(toot, in_reply_to_id:[reply_id], visibility:"unlisted")
    time = Time.now
    File.open("./log/streaming/" + log_count + ".txt", mode = "a+:utf-8:utf-8") do |log|
...
def log_teiki(reply_id)
    ...
    File.open("./log/schedule/" + log_count + ".txt", mode = "r:utf-8:utf-8") do |log|
        log.each {|line|
            toot << line
        }
    end
...
def log_reply(reply_id)
    ...
    File.open("./log/streaming/" + log_count + ".txt", mode = "r:utf-8:utf-8") do |log|
        log.each {|line|
            toot << line
        }
    end
...
begin
    stream.user() do |toot|
        if toot.kind_of?(Mastodon::Notification) then
            if toot.type == "mention" then #メンション(リプライ)か否かを判断
                ...
                elsif content.split[0] == "ログ" and username == "XIG_niconico"
